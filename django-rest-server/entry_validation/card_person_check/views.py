...
class CheckCardPersonView(APIView):
    def post(self, request):
        ...
        if employee:
            employee.current_zone = (
                gate_event.to_zone if result.allowed else employee.current_zone
            )
            employee.save()
...
class AccessRightRequestListView(APIView):
    ...
    def get(self, request):
        ...
        if user.is_supervisor:
            own_requests = models.Q(supervisor=user)

            absent_supervisors = Employee.objects.filter(
                is_supervisor=True, on_the_clock=False, deputy=user
            )
...
class ApproveAccessRightRequestView(APIView):
    ...
    def post(self, request, pk):
        ...
        if (
            not is_assigned_supervisor
            and not is_covering_deputy
            and not is_covering_supervisor
        ):
            return Response(
                {"error": "You are not authorised to approve this request."}, status=403
            )
...
class RegisterEmployee(APIView):
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {"message": "Employee registered successfully."}, status=201
            )
    ...
    def get(self, request):
        ...
        if dept:
            supervisors = Employee.objects.filter(
                is_supervisor=True, department__id=dept, on_the_clock=True
            )
            return Response(
                [
                    {"id": sup.id, "name": f"{sup.firstname} {sup.lastname}"}
...
class ApproveEmployeeRegistrationView(APIView):
    ...
    def post(self, request, pk):
        ...
        employee = Employee.objects.filter(
            pk=pk, is_active=False, supervisor=user
        ).first()
        if not employee:
            return Response(
                {"error": "Employee not found or already active."}, status=404
            )
        ...
        return Response(
            {"message": "Employee registration approved successfully."}, status=200
        )
