...
def check_access_rights(employee: Employee, gate: Gate) -> bool:
    today = timezone.now().date()
    target_zone = gate.opposite_zone
    return (
        ...
        and AccessRight.objects.filter(
            employee=employee,
            security_zone=gate.current_zone,
            start_date__lte=today,
            end_date__gte=today,
        ).exists()
    )
...
def check_card_person(card, employee, gate) -> AccessResult:
    if not card:
        return AccessResult(allowed=False, status_code=404, message="Card not found.")
    if not employee:
        return AccessResult(
            allowed=False, status_code=404, message="Employee not found."
        )
    if not gate:
    ...
    if card.valid_from and card.valid_to:
        if (
            card.valid_from > timezone.now().date()
            or card.valid_to < timezone.now().date()
        ):
            return AccessResult(
                allowed=False,
                status_code=403,
                message="Card is not valid at this time.",
            )
    if card.lock_until and card.lock_until > timezone.now():
        return AccessResult(
            allowed=False, status_code=403, message="Card is temporarily locked."
        )
    if not check_access_rights(employee, gate):
        return AccessResult(
            allowed=False,
            status_code=403,
            message="No valid access rights for this gate.",
        )
